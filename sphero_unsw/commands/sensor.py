...
from sphero_unsw.listeners.sensor import SensorStreamingMask, CollisionDetected, BotToBotInfraredReadings, \
...
class Sensor(Commands):
    ...
    #######################################################################################
    # NEW CODE TO FIX MSG ERROR OF BOLT
    @staticmethod
    def __collision_detected_notify_helper(listener, packet):
        # print(f"Collision packet data: {packet.data.hex()} (length: {len(packet.data)})")
        
        # Use elif and handle 16-byte packets correctly
        if len(packet.data) == 16:
            # BOLT use 16 bytes
            # Format: 3 signed shorts (accel) + 1 byte (axis) + 3 signed shorts (power) + 1 byte (speed) + 1 short (time)
            unpacked = struct.unpack('>3hB3hBh', packet.data)  # Changed original 3hB3hBL to 3hB3hBh
        ...
        elif len(packet.data) == 18:
            # Alternative format if needed (4-byte timestamp)
            unpacked = struct.unpack('>3hB3hBI', packet.data)
            ...
            print(f"Unexpected packet length: {len(packet.data)}")
            print(f"Raw data: {packet.data.hex()}")
            return
