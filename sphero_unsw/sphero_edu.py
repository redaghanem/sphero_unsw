...
from sphero_unsw.commands.animatronic import R2LegActions
from sphero_unsw.commands.io import IO, FrameRotationOptions, FadeOverrideOptions
from sphero_unsw.commands.power import BatteryVoltageAndStateStates
from sphero_unsw.controls import RawMotorModes
from sphero_unsw.helper import bound_value, bound_color
...
class EventType(Enum):
    on_collision = auto()  # [f.Sphero, f.Ollie, f.BB8, f.BB9E, f.R2D2, f.R2Q5, f.BOLT, f.BOLTPLUS,f.Mini]           # NEW CODE TO SUPPORT BOLTPLUS
    on_freefall = auto()  # [f.Sphero, f.Ollie, f.BB8, f.BB9E, f.R2D2, f.R2Q5, f.BOLT, f.BOLTPLUS,f.Mini]           # NEW CODE TO SUPPORT BOLTPLUS
    on_landing = auto()  # [f.Sphero, f.Ollie, f.BB8, f.BB9E, f.R2D2, f.R2Q5, f.BOLT, f.BOLTPLUS,f.Mini]            # NEW CODE TO SUPPORT BOLTPLUS
    on_gyro_max = auto()  # [f.Sphero, f.Mini, f.Ollie, f.BB8, f.BB9E, f.BOLT, f.BOLTPLUS,f.Mini]                   # NEW CODE TO SUPPORT BOLTPLUS
    on_charging = auto()  # [f.Sphero, f.Ollie, f.BB8, f.BB9E, f.R2D2, f.R2Q5, f.BOLT]                          # NEW CODE TO SUPPORT BOLTPLUS
    on_not_charging = auto()  # [f.Sphero, f.Ollie, f.BB8, f.BB9E, f.R2D2, f.R2Q5, f.BOLT]                      # NEW CODE TO SUPPORT BOLTPLUS
    on_magnetometer_north_yaw = auto()  # [f.BOLT]
    on_sensor_streaming_data = auto()  # [f.BOLT]
    on_ir_message = auto()  # [f.BOLT, f.BOLTPLUS,f.RVR] TODO                                           # NEW CODE TO SUPPORT BOLTPLUS
...
class LedManager:
    def __init__(self, cls):
        ...
        elif cls in (R2D2, R2Q5, BOLT, BOLTPLUS):               # NEW CODE TO SUPPORT BOLTPLUS
...
class SpheroEduAPI:
    ...
    def __init__(self, toy: Toy):
        ...
        #################################
        # NEW CODE TO SUPPORT BOLTPLUS
        # --------------------------
        # COLLISION DETECTION STATE 
        # --------------------------
        self.__last_accel_mag = 0.0
        self.__last_speed = 0.0
        self.__last_gyro_mag = 0.0         # For gyro max event
        self.__last_collision_time = 0
        self.__collision_dead_time = 0.20  # Debounce period (seconds), larger value means less sensitive
        self.__accel_threshold = 1.5       # Minimum g-force to register collision, larger value means less sensitive
        self.__speed_drop_threshold = 10.0 # Minimum speed drop (cm/s), larger value means less sensitive
        #################################
    ...
    def __enter__(self):
        ...
        #################################
        # NEW CODE TO SUPPORT BOLTPLUS       
        # Automatically turn off the main LED when connecting (this step required to make all sensors work correctly)
    ...
    def set_stabilization(self, stabilize: bool):
        ...
        if isinstance(self.__toy, (Sphero, Mini, Ollie, BB8, BB9E, BOLT, BOLTPLUS)):            # NEW CODE TO SUPPORT BOLTPLUS
    ...
    def calibrate_compass(self):
        ...
        """
        if isinstance(self.__toy, (BOLT, BOLTPLUS)):                # NEW CODE TO SUPPORT BOLTPLUS
            self.__compass_zero = None
    ...
    def set_front_led(self, color: Color):
        ...
        For Sphero BOLT, BOLTPLUS, R2D2, R2Q5: Changes the color of the front LED light.
        ...
        if isinstance(self.__toy, (R2D2, R2Q5, BOLT, BOLTPLUS, RVR)):           # NEW CODE TO SUPPORT BOLTPLUS
    ...
    def set_back_led(self, color: Union[Color, int]):
        ...
        For Sphero BOLT, BOLTPLUS, R2D2, R2Q5:
        ...
        elif isinstance(self.__toy, (R2D2, R2Q5, BOLT, BOLTPLUS, RVR, Mini)):           # NEW CODE TO SUPPORT BOLTPLUS
    ...
    def __start_capturing_sensor_data(self):
        ...
        elif isinstance(self.__toy, (BOLT, BOLTPLUS)):          # NEW CODE TO SUPPORT BOLTPLUS
    ...
    # Function to set collision sensitivity thresholds
    def set_collision_sensitivity(self, collision_dead_time, accel_threshold, speed_drop):
        """Sets the sensitivity thresholds for collision detection BOLTPLUS.
        :param collision_dead_time: Minimum time (in seconds) between collision events to avoid multiple detections
        :param accel_threshold: Acceleration threshold (in g's) to detect a collision
        :param speed_drop: Speed drop threshold (in cm/s) to detect a collision
        """
        self.__collision_dead_time = collision_dead_time
        self.__accel_threshold = accel_threshold
        self.__speed_drop_threshold = speed_drop



    # Function to process collision detection
    def __process_collision(self):
        """Software-based collision detection for BOLTPLUS using accelerometer, velocity, and gyroscope."""

        # Don't process if stopped (disconnecting/disconnected)
        if self.__stopped.is_set():
            return

        accel = self.__sensor_data.get('accelerometer')
        velocity = self.__sensor_data.get('velocity')
        gyro = self.__sensor_data.get('gyroscope')

        if accel is None or velocity is None or gyro is None:
            return

        # Extract acceleration values (in g)
        ax, ay, az = accel['x'], accel['y'], accel['z']
        accel_mag = math.sqrt(ax * ax + ay * ay + az * az)

        # Extract velocity (in cm/s)
        ...
        # ---------------------------------------------------------------
        # Save previous values for next frame
        # ---------------------------------------------------------------
        self.__last_speed = speed
        self.__last_accel_mag = accel_mag
        self.__last_gyro_mag = gyro_mag
    ...
    def start_ir_follow(self, near: int, far: int):
        ...
        messages from a broadcasting BOLT, BOLTPLUS,the follower will adjust its heading and speed to follow the broadcaster.
